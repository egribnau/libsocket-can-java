...
public final class CanSocket implements Closeable {
	static {
		...
		try {
			...
			try {
				loadLibFromJar(os, arch, LIB_JNI_SOCKETCAN);
	...
	private static void loadLibFromJar(String os, String arch, final String libName) throws IOException {
		Objects.requireNonNull(libName);
		final String fileName = "/lib/" + os + "/lib" + libName + '-' + arch + ".so";
